...
const StateDetails = () => {
  ...
  const updateMapColors = (cityName) => {
    ...
    svgDoc.querySelectorAll("path").forEach((path) => {
      path.style.fill = path.id === cityName ? highlightColor : "#D3D3D3"; // Light gray for unselected cities
    });
  };
    ...
    <div className="min-h-screen" style={{
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* Left Side - Map */}
          <div className="relative">
            <object ref={svgRef} type="image/svg+xml" data="/images/states/himachalpradesh/details.svg" className="w-full h-full cursor-pointer" />
          </div>
