      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-12">
            <button className="text-xl font-['Crimson']">Save Itenary</button>
            <Link to="/hotel_booking" className="bg-gradient-to-r from-cyan-500 to-green-400 text-black px-8 py-3 rounded-full flex items-center gap-2">
              Let's Book
              <span>→</span>
            </Link>
          </div>
       ...
       {/* Days Grid */}
<div className="grid grid-cols-3 gap-8 p-6 overflow-auto bg-scroll-black scrollbar-thick scrollbar-thumb-black scrollbar-track-transparent">
  {days.map((day) => (
    <div 
      key={day.day} 
      className="bg-gradient-to-b from-emerald-400 via-emerald-300 to-cyan-300 p-8 relative h-[600px] shadow-lg rounded-lg"
      style={{
        boxShadow: '-10px 12px 30px rgba(0, 0, 0, 0.6)', // Bigger shadow effect
      }}
    >
      {/* Day Number */}
      <h2 className="text-[3.5rem] font-normal mb-6">Day {day.day}</h2>
      ...
      {/* Places List - Added Scrolling */}
      <div className="space-y-6 overflow-y-auto max-h-[350px] pr-4 scrollbar-thick scrollbar-thumb-black scrollbar-track-transparent">
        {day.places.map((place, index) => (
          <div key={place.id} className="flex items-center gap-4 pb-4 relative scrollbar-thick scrollbar-thumb-black scrollbar-track-transparent">
            {/* Number Circle */}
            <span className="w-10 h-10 bg-black text-white rounded-full flex items-center justify-center text-xl">
              {place.id}
            </span>
            ...
            {index < day.places.length - 1 && (
              <div className="absolute left-5 top-10 w-1 h-10 bg-black"></div>
            )}
