...
//     svgDoc.querySelectorAll("path").forEach((path) => {
//       path.style.fill = path.id !== stateName ? "#D3D3D3": ""; // Light gray for unselected states
//     });
//   };
  ...
  const updateMap = (stateName) => {
    ...
    svgDoc.querySelectorAll("path").forEach((path) => {
      if (path.id === stateName) {
        path.style.fill = "#1BC8FF"; // Highlight selected state
        ...
        path.style.transform = "scale(1.2)"; // Scale up the selected state
      } else {
        path.style.fill = "#D3D3D3"; // Reset color for unselected states
        path.style.transform = "scale(1)"; // Reset scale for unselected states
      }
    });
  
    // Create or update text label
    let textLabel = svgDoc.querySelector("#stateLabel");
    if (!textLabel) {
      textLabel = document.createElementNS("http://www.w3.org/2000/svg", "text");
      textLabel.setAttribute("id", "stateLabel");
      textLabel.setAttribute("fill", "black");
      textLabel.setAttribute("font-size", "20");
      textLabel.setAttribute("font-weight", "bold");
      textLabel.setAttribute("text-anchor", "middle");
      svgDoc.documentElement.appendChild(textLabel);
    }
  
    const selectedPath = svgDoc.getElementById(stateName);
    if (selectedPath) {
      const bbox = selectedPath.getBBox();
      textLabel.setAttribute("x", bbox.x + bbox.width / 2);
      textLabel.setAttribute("y", bbox.y + bbox.height / 2);
      textLabel.textContent = stateName;
    }

      // Delay navigation by 2 seconds
  setTimeout(() => {
    navigate(`/state/${stateName.toLowerCase().replace(/\s+/g, "_")}`);
  }, 1500);
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <p className="text-lg font-medium font-['CrimsonText'] text-center underline">
                  Select a State
                </p>
                <div className="relative w-full sm:w-64 mx-auto">
                  <button
                ...
                <button
                  onClick={() => {
                    selectedState &&
                      navigate(`/state/${selectedState.toLowerCase().replace(" ", "_")}`);
                  }}
                  className="bg-[#1BC8FF] text-black px-10 py-2 rounded-full hover:bg-[#00916E] hover:text-white transition-colors flex items-center gap-2 mx-auto"
                >
                  Explore
